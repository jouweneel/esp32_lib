...
static const framesize_t sizes[6] = { FRAMESIZE_QVGA, FRAMESIZE_CIF, FRAMESIZE_VGA, FRAMESIZE_SVGA, FRAMESIZE_XGA, FRAMESIZE_UXGA };

SemaphoreHandle_t fb_request = NULL;
bool synced = false;
...
static int i2s_run()
{
    ...
    if (synced == true) {
      BaseType_t xGotVsync = xSemaphoreTake(fb_request, 1);
      if (xGotVsync > 0) {
        i2s_start_bus();
        return 0;
      }
      ESP_LOGE(TAG, "Vsync semaphore missed");
    }
...
static void IRAM_ATTR vsync_isr(void* arg)
{
    ...
    if (_gpio_get_level(s_state->config.pin_vsync) == 0) {
        if (uxSemaphoreGetCount(fb_request) == 0) {
            BaseType_t xHigherPriorityHasWoken = 0;
            xSemaphoreGiveFromISR(fb_request, &xHigherPriorityHasWoken);
        }
...
esp_err_t camera_init(const camera_config_t* config)
{
    ...
    fb_request = xSemaphoreCreateBinary();
...
camera_fb_t* esp_camera_fb_get()
{
    ...
    xSemaphoreTake(fb_request, portMAX_DELAY);
...
esp_err_t esp_camera_set(CameraConfig_t *settings) {
  esp_err_t err = ESP_OK;
  framesize_t frame_size = sizes[settings->frame_size];

  int qp = settings->jpeg_quality;
  int compression_ratio_bound = 1;
  if (qp > 10) {
      compression_ratio_bound = 16;
  } else if (qp > 5) {
